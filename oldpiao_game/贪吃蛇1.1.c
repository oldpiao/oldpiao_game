...
int snake2(int p6[M][N],int x6,int y6,int *t6,int *b_x,int *b_y,int *bjm_p6[])
{
	...
	while(1)//bjm_gz 游戏规则
	{
		...
		while(!kbhit()||s==1)//如果不点击就循环
		{
			...
			switch(ch)
			{
			case 'a': s=bjm_gz2(p6,x6,y6,p6[*b_x][*b_y-1],t6);
					  if(s==1)
						  return 1;
					  p6[*b_x][*b_y-1]=1;
					  if(s!=2)
			...
			case 'd': s=bjm_gz2(p6,x6,y6,p6[*b_x][*b_y+1],t6);
					  if(s==1)
						  return 1;
					  p6[*b_x][*b_y+1]=1;
					  if(s!=2)
			...
			case 'w': s=bjm_gz2(p6,x6,y6,p6[*b_x-1][*b_y],t6);
					  if(s==1)
						  return 1;
					  p6[*b_x-1][*b_y]=1;
					  if(s!=2)
					  ...
					  {
						  (*t6)++;
						  bjm_p6[(*t6)-1]=&p6[*b_x-1][*b_y];
						  ball(p6,x6,y6);//创建小球
						  (*b_x)--;
						  break;
					  }
					  
			case 's': s=bjm_gz2(p6,x6,y6,p6[*b_x+1][*b_y],t6);
					  if(s==1)
						  return 1;
					  p6[*b_x+1][*b_y]=1;
					  if(s!=2)
